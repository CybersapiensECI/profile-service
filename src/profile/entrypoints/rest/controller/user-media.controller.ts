...
import { USER_MANAGEMENT_SERVICE_PORT, USER_MEDIA_SERVICE_PORT } from '../../../domain/ports/injection-tokens';
...
export class UserMediaController {
  ...
  @ApiOperation({
    ...
    description: 'Accepts a multipart file. Validates readability and content type.',
  })
  @ApiConsumes('multipart/form-data')
  @ApiBody({ schema: { type: 'object', properties: { file: { type: 'string', format: 'binary' } } } })
  @ApiResponse({ status: 200, description: 'Profile image updated successfully' })
  @ApiResponse({ status: 400, description: 'File is missing, unreadable or invalid format' })
  ...
  async uploadProfileImage(
    ...
    if (!file) {
      throw new BadRequestException('It was not possible to read the file. Please try again.');
    }
    return this.mediaService.updateProfileImage(userId, file.buffer, file.mimetype);
  }
  ...
  @ApiOperation({
    ...
    description: 'Redirects (302) to the stored profile image URL for the given user.',
  })
  @ApiResponse({ status: 302, description: 'Redirect to the profile image URL' })
  @ApiResponse({ status: 400, description: 'User does not have a valid profile image' })
  @ApiResponse({ status: 404, description: 'User not found' })
  async getProfileImage(@Param('userId') userId: string) {
    const user = await this.managementService.getUser(userId);

    if (!(user instanceof StudentProfileResponseDto) || !user.photoUrl?.trim()) {
      throw new BadRequestException('This user does not have a valid profile image.');
    }
  ...
  @ApiResponse({ status: 200, description: 'Geolocation setting updated successfully' })
  ...
  async updateGeolocation(
    ...
    return this.mediaService.updateGeolocation(userId, request.geolocationEnabled);
  }
}
